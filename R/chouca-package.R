...
#'  
#'  The workflow of working with this package has typically four steps (1) define the 
...
#'    }
#'    
#'    This model defines two probabilities of transition (between the "bare" and the 
#'    "plant" state and vice versa), hence implicitely two states. The first probability 
#'    depends on the proportion of "plant" cells in the landscape, \code{p["plant"]}, and 
#'    the proportion of neighbors in the "plant" state, \code{q["plant"]}. It has three 
#'    parameters, \code{r1}, \code{r2} and \code{m}. The \code{neighbors} argument 
#'    defines the type of neighborhood (here 8-way, or Moore neighborhood), and we 
#'    specify that the model should run over a toric space that wraps around the edges of 
#'    the grid. 
#'    
#'    More information about the creation of models is available in \code{\link{camodel}}. 
#'    
...
#'    you would like to use, we recommend to process it first using
#'    \code{\link{as.camodel_initmat}} so it will play nicer with the rest of the package
#'    functions. 
#'    
#'  (3) Running the model 
#'    
...
#'    and the model output are saved and are documented in the function help page. 
#'    
...
#' as the simulation is run (see e.g. \code{\link{landscape_plotter}}), use different 
