...
devtools::install_github("alexgenin/chouca", upgrade = FALSE)
library(chouca)
...
invisible({ 
  run_camodel(mod, init, times = seq(0, 164), 
...
$P(0 \rightarrow +) = 0.1 + sin(p_i)$ may not be correctly handled (see below for the consequences). 
...
P( a \rightarrow b ) = \beta_0 \\ 
  ...
  + \gamma_1 q_1 q_2 + q_1 q_2^2 + q_1^2 q_2 ... \gamma_n q_{S-1}^k q_S^k \\
...
function of $q_s$. All the $(\alpha_i)$, $(\beta_i)$, $(\gamma_i)$ and $(\delta_i)$ are 
constants (model parameters). Many models can be represented using this functional form - 
however, this may not always be the case, especially as soon as non-linear 
functions are involved (exp, sin, etc.), or functions with discontinuities. This 
functional form can be daunting, but `chouca` will warn you if the transition 
probabilities cannot be accurately modelled, so the best way is to write your model and 
...
One of the main appeal of `chouca` is its performance: which is affected by various 
factors. The main one is the model complexity. Using a model with transition 
probabilities that are complicated products of global or neighbor covers, or that 
do not fit well the functional form expected will lead to poor performance. When 
possible, try to make your model follow those forms. However, several approaches exist 
to improve simulation speed in chouca, regardless of the model: (i) on-the-fly 
compilation, (ii) memoisation of transition probabilities and (iii) multithreading. 

On-the-fly compilation can be enabled using the "compiled" engine to run the cellular 
automaton. This is done by setting the `engine` control argument to "compiled": 
...
size of the landscape changes. A simple change in parameter value will not trigger a 
new compilation, except when the probabilities of transition can be simplified, for 
example when a parameter goes to zero and part of the equation can be left out. 
...
To avoid this, we can compute the probabilities of transition for every neighborhood 
...
Below is a summary of performance for the forest gap model, different options and 
landscape sizes: 

```{r benhmarks, results = FALSE}
...
Various options are available to access simulation outputs. When nothing is specified, 
the percentage of cells in each state will be saved at each time step specified 
in the `times` vector (argument to `run_camodel()`), and the landscape at the first and 
last time step will be saved. These can be accessed simply by acessing elements of the 
object returned by `run_camodel()`. 

Global covers are saved as a matrix whose first column is the time, and the others the 
...
The landscape are also available, using the following syntax. A nifty function to 
display landscapes is available in the spatialwarnings package, `display_matrix`: 
...
Sometimes, we do not want to store everything at all time steps. One option to reduce 
output is to specify a diferrent `times` vector to `run_camodel()`. This will not 
affect the precision of the simulation, but will simply reduce the time points at which
...
plot(covers[ ,"t"], covers[ ,"forest"], type = "l", col = "red")
lines(covers2[ ,"t"], covers2[ ,"forest"], col = "blue")
```

Another option is to set the control argument `save_covers_every` to something higher 
than one. This will only save simulation output one out of several iterations: 
...
A similar argument exists for saving landscapes, `save_snapshots_every`, to reduce 
or increase the number of landscapes being saved during the simulation, with similar 
effects. 
...
One of the easiest way to dissect the results of your simulation is simply to store 
the landscape, or the covers of each state, then run your computation after the fact. 
This is not always possible, however, as the landscapes may be large, or your model 
may be running for a large number of iterations. To avoid this, chouca can take 
a special function that will be run at regular intervals as the simulation is running. 

This function must have a specific form: its first argument should be `t`, the current 
time of the simulation, and the second should be `mat`, the current landscape. 

For example, we can compute how the spatial autocorrelation changes in the landscape 
as the simulation is running, measured by the Moran's I index (available in package 
(spatialwarnings)[https://cran.r-project.org/package=spatialwarnings]): 
...
The results can be extracted the following way: 

```{r}
my_output <- out[["output"]][["custom"]]
```

By default, all custom outputs are stored in a list, so a common operation is to 
transform it into a table using `rbind` or any other function: 
...
The use of custom functions has the nice side-effect that simulations can be visualised 
as they are running. This can be done, again by passing a specific function to `chouca`. 
The package includes a few helpers to do so. 

The first one is `landscape_plotter()`, which can be used as follow, and will display 
the landscape as it is being run. We use the "rock-paper-scissor" model included with 
chouca as example for its nice results: 

```{r giflandscape, output = FALSE, fig.show = "animate", fig.width = 4, fig.height = 4, dev='jpeg', interval = 0.04, cache = TRUE, results = FALSE}
...
Another option to visualise simulations as they are run is using `trace_plotter()`, 
which will display the covers of each state in the landscape during the run: 

```{r giftraces, output = FALSE, fig.show = "animate", fig.width = 4, fig.height = 4, dev='jpeg', interval = 0.04, cache = TRUE, results = FALSE}
...
These two functions have many options to customize their output to your liking, which 
are documented in `?trace_plotter` or `?landscape_plotter`. Of course using them involves 
a large performance penalty as the plots needs to be redrawn during the execution of the 
simulation. 
...
camodel(transition(from = "0", to = "+", ~ 1 + exp( 2 + 2 * p["+"] * q["+"])), 
        wrap = TRUE, 
        neighbors = 4)
```
...
```{r model_error_ok}
exp_approx <- function(x) 1 + x + x^2/2 + x^3/(3*2) + x^4/(4*3*2)
