                ...
                for (arma::uword i = 0; i < nr; i++) {
                    ...
                    for (ushort to = 0; to < ns; to++) {
                        ...
                        for (arma::uword k = 0; k < beta_0_index.n_rows; k++) {
                            ptrans(to) += (beta_0_index(k, _from) == from) * (beta_0_index(k, _to) == to) * beta_0_vals(k);
                        }
                        ...
                        for (arma::uword k = 0; k < beta_q_index.n_rows; k++) {
                            ...
                            ptrans(to) += (beta_q_index(k, _from) == from) * (beta_q_index(k, _to) == to) *
                                // Given the observed local abundance of this state, which line in
                                // beta_q should be retained ?
                                (beta_q_index(k, _qs) == qthis) * beta_q_vals(k);
                        }
                        ...
                        for (arma::uword k = 0; k < beta_pp_index.n_rows; k++) {
                            ...
                            ptrans(to) += (beta_pp_index(k, _from) == from) * (beta_pp_index(k, _to) == to) * beta_pp_vals(k, _coef) * pow(p1, beta_pp_index(k, _expo_1)) * pow(p2, beta_pp_index(k, _expo_2));
                        }
                        ...
                        for (arma::uword k = 0; k < beta_qq_index.n_rows; k++) {
                            ...
                            ptrans(to) += (beta_qq_index(k, _from) == from) * (beta_qq_index(k, _to) == to) * beta_qq_vals(k, _coef) * pow(q1, beta_qq_index(k, _expo_1)) * pow(q2, beta_qq_index(k, _expo_2));
                        }
