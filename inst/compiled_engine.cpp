...
// Information about absorbing states, i.e. states we will never get out of
...
void aaa__FPREFIX__camodel_compiled_engine(const arma::Mat<ushort> all_qs_arma,
    ...
    while (current_t <= last_t) {
        ...
        for (uword s = 0; s < substeps; s++) {
            ...
            // This has funny identation but it is the only way to align code in the
  ...
  private(ptrans) reduction(+:new_ps)
            ...
            for (uword i = 0; i < nr; i++) {
                ...
                for (uword j = 0; j < nc; j++) {
                    ...
                    // skip the checking of this cell. This should be optimized away
                    // by the compiler if there is no absorbing state, because
                    // is_absorbing_state() will return a constant (false).
