...
large and not that many may fit in your computer's memory, or your model may be running 
for a large number of iterations. To avoid this, `chouca` can take a special function
that will be executed as the simulation is running.
...
* While `chouca` is flexible, it would probably be unwise to define stochastic cellular 
automata with more than 100 different states. `chouca` works better with models that 
have a low number of states and/or transitions. 
