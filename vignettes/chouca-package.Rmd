...
```{r gifcreate, echo = FALSE, output = TRUE, fig.show = "animate", fig.width = 4, fig.height = 4, dev='jpeg', interval = 0.04, cache = TRUE}

if ( ! requireNamespace("animate", quietly = TRUE) ) { 
...
# Make sure we upload the files
if ( file.exists("upload_files.R") ) { 
  source("upload_files.R")
}

```

Please see animation <a href="https://alex.lecairn.org/files/chouca_sca_gifcreate.webm">there</a> if it does not show in your viewing software.

Stochastic cellular automata are widely used to describe the dynamics of
landscapes, for example, forests or coral reefs growing over space. Yet,
...
[rock-paper-scissor automaton](https://www.youtube.com/watch?v=TvZI6Xc0J1Y). A
deterministic cellular automaton is simply a subclass of stochastic celular
automaton in which probabilities of transition are always ones or zero.
...
You can also plot the model structure (the states and the transitions), using 
...
constant model parameters. Many models can be represented using this functional form -
...
them afterwards. This is not always possible, however, as the landscapes may be
...
## Visualising simulations as they run
...
```{r trace_landscape, echo = TRUE, output = TRUE, fig.show = "animate", fig.width = 4, fig.height = 4, dev='jpeg', interval = 0.04, cache = TRUE}
...
Note that we use the `new_window = FALSE` argument above so that the plots are 
displayed on the default graphic device, instead of doing it in a new window, the 
default behavior (this is required to produce this vignette).
...
```{r trace_covers, echo = TRUE, output = TRUE, fig.show = "animate", fig.width = 4, fig.height = 4, dev='jpeg', interval = 0.04, cache = TRUE}
