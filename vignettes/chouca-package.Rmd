...
`chouca` works only with models in which the effects are isotropic, i.e. all neighbors
have a similar effect regardless of whether they are above, or below a given cell.
Cellular automata that involve a preferencial direction cannot be implemented in
`chouca`, for example with water running downslope in a landscape (Mayor et al. 2013).
...
An important notion in a cellular automaton is the *neighborhood* of a cell. It is
defined in `chouca` by a *neighborhood kernel*, which is a matrix with TRUE/FALSE
(logical) values that defines whether a neighboring cell is included or not within
the neighborhood.

This matrix must be square and have an odd number of rows and columns. The center cell
of this kernel is always FALSE and represents the focal cell. The other cells of the
kernel matrix represent neighbors to the south/north/east/west of the focal cell. TRUE
values mean that the neighbor at this relative position will be included in the
cell neighborhood.

In `chouca`, this determines the way the $q_s$ are computed, which are the proportions
of cells in state $s$ around the focal cell.

Typical neighborhoods include the Moore (8x8) or Von-Neumann (4x4) of order 1 (only
nearest neighbors of a cell), which can be defined by
`nb_kernel(type = "moore", distance = 1)`, or
`nb_kernel(type = "von-neumann", distance = 1)`.
You can also use `8` or `4` as a shorthand for these kernels.
...
mod <- camodel(transition(from = "dead", to = "live",
               ...
               transition(from = "live", to = "dead",
                          ~ 0.1),
               wrap = TRUE,
               neighbors = nb_kernel(type = "moore", distance = 1))
print(mod)
```

`chouca` includes several types of neighborhood kernels, that can be defined using the
helper function `nb_kernel()`, but you may also construct a neighborhood matrix by
hand. Below is a display of a few kernels generated using `nb_kernel()`:
...
One of the main goal of `chouca` is performance, which is affected by
...
structure of the model, the size of the landscape or the type of neighborhood changes. A
simple change in parameter value will not trigger a new compilation, except when the
...
state it could switch to. This entails a lot of repeated computations, as
...
configuration, and just look up the number to know whether a cell switches or not.
...
This can dramatically improve the performance for models that have a small number of states and simple transition rules. However, it may be counter-productive for models with a larger number of states, as the number of neighborhood configurations grows very quickly. A simple 4x4 or 8x8 neighborhoods has a number of neighbor configurations of $S^4$ or $S^8$ for $S$ states, and much more with larger neighborhoods that consider cells away from a focal cell. By default, a simple heuristic is used to make a decent choice.
...
not that many may fit in your computer's memory, especially if your are running your model for a large number of iterations. To avoid this, `chouca` can take a special function that will be executed as the simulation is running.
...
### I am getting the warning "Unloading chouca DLLs because we reached the limit of 20"

If you use the `compiled` backend, `chouca` creates a shared object file (or "library",
DLL), that is loaded by R and makes new C++ functions available to the package. As you carry on with your session and if you compile many different models, the number of loaded DLLs by chouca may become quite high and reach the limit set by R (typically 100).

To avoid this situation, `chouca` will not load more than 20 DLLs, and unload all DLLs it
has loaded so far if that limit is reached. This is mostly harmless, but means that previous models will need to be recompiled. You can increase the limit using `options(chouca.maxdll = <n>)`, or disable the warning altogether using
`options(chouca.maxdll.warn = FALSE)`.

Note that `chouca` does not produce one DLL per model, as a simple change in model
parameters usually does not trigger a recompilation and the same DLL is reused.
